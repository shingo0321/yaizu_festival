...
- `mikoshi-route.md` — human-readable list of the mikoshi route under two headings, `# 往路` (outbound) and `# 帰路` (return), each stop with one or more Google Maps share links (`maps.app.goo.gl/...`). The first link under each named stop is the source of truth for that stop's entry in `data.js`'s `mapPins.points` (往路) / `mapPins.pointsReturn` (帰路) — `{ label, lat, lng, mapUrl }`, in order; bare links with no stop name above them are reference-only and not reflected there. Like `mikoshi-route.dio`, this is a live file the owner may edit directly between (or within) sessions.
  - `mapPins.routeMapArtifactUrl` links the "往路の地図を見る" / "帰路の地図を見る" buttons (via `#outbound` / `#return` anchors) to a Claude Artifact that plots the *full* `mikoshi-route.md` coordinates for each leg — including the bare/unnamed links that `points`/`pointsReturn` skip — as a straight-line overlay on an OpenStreetMap basemap (no road-snapping, unlike a Google Maps directions link). The artifact is a standalone page built and republished manually when the route changes; there's no script here that regenerates it. **Artifacts are private by default** — the owner must share it (via the artifact page's share menu) for site visitors to actually open the link.
