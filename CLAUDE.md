...
- `mikoshi-route.md` — human-readable list of the mikoshi route under two headings, `# 往路` (outbound) and `# 帰路` (return), each stop with one or more Google Maps share links (`maps.app.goo.gl/...`). The first link under each named stop is the source of truth for that stop's entry in `data.js`'s `mapPins.points` (往路) / `mapPins.pointsReturn` (帰路) — `{ label, lat, lng, mapUrl }`, in order; bare links with no stop name above them are reference-only and not reflected there. Like `mikoshi-route.dio`, this is a live file the owner may edit directly between (or within) sessions.
  - `mapPins.routeWaypoints` / `routeWaypointsReturn` mirror the *full* sequence of `mikoshi-route.md` coordinates for each leg — including the bare/unnamed links that `points`/`pointsReturn` skip — as plain `[lat, lng]` pairs. These feed the "◯路をGoogleマップで見る" buttons (`directionsUrl()` in `app.js`) so the walking directions follow the actual recorded path, not just a straight line between the named stops.
  - `directionsUrl()` builds a `/maps/dir/lat,lng/lat,lng/.../` URL and forces walking directions via a `data=!4m2!4m1!3e2` suffix — the simpler `?travelmode=walking` query param only works on the `?api=1` URL form, not this path-based one.
