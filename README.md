...
焼津祭の「開催日程・タイムテーブル」「会場マップ・アクセス」を掲載するポータルサイト。
...
[data.js](data.js) 内の `TODO:` と書かれた箇所を、実際のタイトル・日程・会場・アクセス情報に書き換える。
...
- 会場マップはGoogle MapsのURL埋め込み（APIキー不要）を使用しているため追加費用は発生しない
